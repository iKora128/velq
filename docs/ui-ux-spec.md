...
- **HTML / `.velq` viewing** opens in the isolated WebView (per `requirements.md` §5). **HTML editing defaults to the Rendered page itself** *(updated 2026-07-04, W6/M27: what a browser only displays, Velq edits in place — text, new paragraphs, ⌘B/I/U all write back to the exact source range; head/doctype untouched)*, with **Split (source + preview) and Source one toggle away** for attributes, styles and markup surgery. → Rendered suits content; split suits markup.
- **Preview templates** *(added 2026-07-05, M29)*: the rendered-Markdown look is a switchable template — **Paper** (calm serif, the original), **Docs** (the GitHub look), **Note** (Notion/Bear-style: accent-bar H2s, callout quotes, rounded tables), **Magazine** (editorial: display headings, accent bands, marker-highlighted bold), **Tech** (Zenn-style article card on blue-gray), **Sky** (Nani-style: sky-blue pills, extra-round card, dotted links), **Glass** (tategazou-style frosted card over a teal/orange/red gradient mesh). One persisted choice, applied everywhere rendered Markdown appears — the split preview, Quick Look, and HTML/PDF exports — so an export always looks like the preview it came from. Switch via the palette icon in the editor toolbar (split view) or Settings → Appearance. HTML documents are never templated; they carry their own styles. Template names stay generic in the UI — the inspirations are logged here, not shipped as brand names. *(Bear themes + Typora CSS themes, kept to a handful of opinionated choices instead of a theme store)*
