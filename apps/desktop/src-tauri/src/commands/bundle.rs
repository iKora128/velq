...
/// A sibling `<stem>.velq` beside `src` (`/vault/Notes.md` → `/vault/Notes.velq`),
/// de-duplicated so an existing package is never clobbered. Where an **in-place**
/// conversion writes: right next to the source, in the same folder — no copy off
/// in a staging directory.
fn sibling_velq(src: &Path) -> PathBuf {
    let dir = src.parent().unwrap_or_else(|| Path::new("."));
    let stem = src
        .file_stem()
        .map(|s| s.to_string_lossy().into_owned())
        .unwrap_or_else(|| "Document".into());
    unique_velq(dir, &stem)
}

/// Convert a Markdown file to a `.velq` **in place**: write a sibling `<stem>.velq`
/// (source + rendered/enriched HTML + bundled assets) and remove the original `.md`.
/// EXPLICIT action only — the user invokes "Convert to .velq"; this NEVER runs as a
/// side effect of opening a file. The frontend renders + enriches (so it can show
/// progress); this bundles, zips, and swaps. Returns the new path and what it
/// collected. Referenced images are copied INTO the package but left on disk (they
/// may be shared), so only the source document itself is removed.
#[tauri::command]
pub async fn convert_md_in_place(
    md_path: String,
    md: String,
    html: String,
) -> Result<PackagedVelq, String> {
    let src = Path::new(&md_path).to_path_buf();
    let out = sibling_velq(&src);
    let out_str = out.to_string_lossy().into_owned();
    ...
    std::fs::remove_file(&src)
        .map_err(|e| format!("packaged, but couldn't remove the original: {e}"))?;
    Ok(PackagedVelq {
        out_path: out_str,
        collected: result.report.collected,
        failed: result.report.failed.len(),
...
/// removed). EXPLICIT action only — never a side effect of opening. Returns the new
/// path and what it collected.
#[tauri::command]
pub async fn convert_html_in_place(html_path: String) -> Result<PackagedVelq, String> {
    let src = Path::new(&html_path).to_path_buf();
    let out = sibling_velq(&src);
    let out_str = out.to_string_lossy().into_owned();
    let report = pack_bundled(&html_path, &out_str, true).await?;
    std::fs::remove_file(&src)
